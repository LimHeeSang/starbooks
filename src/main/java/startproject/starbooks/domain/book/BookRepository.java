...
public interface BookRepository extends JpaRepository<Book,Long>, QuerydslPredicateExecutor<Book> {
    ...
    @Query(value = "select A.* , B.average " +
            ...
            "limit :start, :offset"
            ,nativeQuery = true)
    List<Book> findAllByOrderByStarRate(@Param("start") int start, @Param("offset") int offset);
    ...
    @Query(value = "select A.*, B.total " +
            ...
            "limit :start, :offset"
            ,nativeQuery = true)
    List<Book> findAllByOrderByHeart(@Param("start")int start, @Param("offset")int offset);
}